...
(* |check_sub| -- check an  expression with sub and return its type *)
let check_sub tv ti = 
  match tv with
      Array (s, t) ->
        if ti == Integer then t else failwith "the index must be an integer"
    | _ -> failwith "cannot use sub with no-array type"
...
let rec check_stmt s env =
  match s with
    ...
    | Assign (lhs, rhs) ->
        ...
        if ta <> Integer && ta <> Boolean then sem_error "assignment can only deal with integer and boolean" [];
